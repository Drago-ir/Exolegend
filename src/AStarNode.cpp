...
tuple<int, int> AStarNode::runAStar(unsigned long time_budjet){
    ...
    do{
        //std::cout << "Made One Iteration ! " << std::endl;
        //pSelectedLeafNode->printNode();

        int costToSelectedNode;
        childIndex = -1;
        //int descendanceSize = pSelectedLeafNode->getDescendanceSize();

        //std::cout << descendanceSize << std::endl;
        ...
        if (millis() - startTime > time_budjet){
            return {-1,-1};
        }
...
void AStarNode::generateChild(int childIndex){
...
int AStarNode::getResidualCost(){
    ...
    //NOTE : debug de-activated second A Star
    ...
    int friendLocation = this->teams[this->teamTakingItsTurn].robots[true].location;

    if(abs(friendLocation%MAZE_WIDTH - robotLocation%MAZE_WIDTH) < 3 && abs(friendLocation/MAZE_WIDTH - robotLocation/MAZE_WIDTH) < 3){
        residualCost += 1000;
        return residualCost;
    }
    
    /*
    if (this->timeUntilGasClosing < 3){
        residualCost += 2;
        //return residualCost;
    }
    */
    ...
    /*
    //NOTE : beware, next few lines causing panic error on esp32 --> try debug if enough time
    //if number of clusters increases
    if (this->pParentNode != nullptr && getNumberOfCoinClusters() > this->pParentNode->getNumberOfCoinClusters()){
        //TODO : may be tuned
        residualCost += 1;
    }
    */
    ...
    int costForDistanceToBorder = (MAZE_WIDTH/2 - this->gasClosures) - (getDistanceToBorder() - 1);
    if (costForDistanceToBorder < 2){
        costForDistanceToBorder = 0;
    }

    /*
    int costForDistanceToBorder = 0;
    if (getDistanceToBorder() - 1 <= 2){
        costForDistanceToBorder = 4;
    }
    */
    //TODO : may be tuned
    residualCost += costForDistanceToBorder;

    if(this->maze[robotLocation] & BEAUTIFUL_COIN_MASK){
        if (residualCost > 2){
            residualCost -= 2;
        }
        else{
    ...
    if(this->maze[robotLocation] & MARVELOUS_COIN_MASK){
        if (residualCost > 4){
            residualCost -= 4;
        }
        else{
...
int AStarNode::getDistanceToBorder(){
    ...
    int minDistToGas = *min_element(distToBorders, distToBorders + 4) - this->gasClosures;
    return minDistToGas;
...
bool AStarNode::isPossibleToReachMazeCenter(){
    ...
    //run A Star
    std::tie(costToLonelyCoin, indexToLonelyCoin) = nodeWithOneCoin.runAStar(10000);
